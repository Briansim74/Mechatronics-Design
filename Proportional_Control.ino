...
void loop() {
  ...
  if (deltaT) {
      ...
      //D = Kp*(omegaDes - omegaSpeed); //Proportional Control Loop Code
      SumError = ((omegaDes - omegaSpeed) + SumError);
      if (SumError < 80) {
          D = (Kp*(omegaDes - omegaSpeed) + Ki*SumError/100);
      }
      else {
          D = (Kp*(omegaDes - omegaSpeed) + Ki*80/100);
      }
...
void plotControlData() {
  Serial.println("Speed, Desired_Speed, PWM_Duty/10");
